...
public class 摆脱Hotkey : IHotkeyResolver
{
    public void Draw(Vector2 size)
    {
        HotkeyHelper.DrawSpellImage(size, Spells.摆脱);
    }

    private static int _check()
    {
        if (!Spells.摆脱.GetSpell().IsReadyWithCanCast()) return -1;
    ...
    public void DrawExternal(Vector2 size, bool isActive)
    {
        ...
        HotkeyHelper.DrawCooldownText(Spells.摆脱.GetSpell(), size);
        HotkeyHelper.DrawChargeText(Spells.摆脱.GetSpell(), size);
    }
