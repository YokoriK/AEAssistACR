...
public class 勇猛Hotkey : IHotkeyResolver
{
    public void Draw(Vector2 size)
    {
        HotkeyHelper.DrawSpellImage(size, Spells.勇猛);
    }

    private static int _check()
    {
        if (!Spells.勇猛.GetSpell().IsReadyWithCanCast()) return -1;
    ...
    public void DrawExternal(Vector2 size, bool isActive)
    {
        ...
        HotkeyHelper.DrawCooldownText(Spells.勇猛.GetSpell(), size);
        HotkeyHelper.DrawChargeText(Spells.勇猛.GetSpell(), size);
    }

    public void Run()
    {
        AI.Instance.BattleData.AddSpell2NextSlot(Spells.勇猛.GetSpell());
    }
}